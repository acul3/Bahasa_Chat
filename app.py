...
tokenizer, model, device = load_tokenizer_and_model(
    "Bahasalab/BahasaGpt-chat", load_8bit=True
)
...
def predict(
    ...
    with torch.no_grad():
        for x in sample_decode(
            ...
            model,
            tokenizer,
            stop_words=["[|Manusia|]", "[|AI|]"],
            max_length=max_length_tokens,
            temperature=temperature,
            top_p=top_p,
        ):
            if is_stop_word_or_prefix(x, ["[|Manusia|]", "[|AI|]"]) is False:
                if "[|Manusia|]" in x:
                    x = x[: x.index("[|Manusia|]")].strip()
                if "[|AI|]" in x:
...
demo.title = "BahasaGPT"

if __name__ == "__main__":
    reload_javascript()
    demo.queue().launch(
        share=True, favicon_path="./assets/favicon-bahasa.png", inbrowser=True
    )